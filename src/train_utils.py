...
class SemiSynthetic(data.Dataset):
    ...
    def __init__(self, root, S_shape):
        super(Synthetic, self).__init__()
        # Rademacher Distribution
        self.S = 2 * torch.randint(0, 2, S_shape) - 1
        ...
        self.A = torch.tensor(self.labels)

        self.train_data = torch.matmul(self.A.float(), self.S.float()) + (0.1**0.5)*torch.randn(9025, 100)
...
def get_dataloader(BATCH_SIZE: int, syn: bool, semi: bool, DIR):
    ...
    if syn:
        if semi:
            source_domain = SemiSynthetic(root=DIR, S_shape=(3, 100))
            source_dataloader = torch.utils.data.DataLoader(source_domain, BATCH_SIZE,
                                                        worker_init_fn=seed_worker(41))
        else:
            source_domain = Synthetic(A_shape=(2500, 5), S_shape=(5, 200))
            source_dataloader = torch.utils.data.DataLoader(source_domain, BATCH_SIZE,
                                                        worker_init_fn=seed_worker(41))
    else:    
